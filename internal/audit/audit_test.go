...
func TestWrite_CreatesFileAndAppendsEntries(t *testing.T) {
	...
	f, err := os.Open(logPath) //nolint:gosec // test path derived from t.TempDir(), not user input
	if err != nil {
...
func TestWrite_TsIsRFC3339(t *testing.T) {
	...
	f, _ := os.Open(logPath) //nolint:gosec // test path derived from t.TempDir(), not user input
	defer f.Close()          //nolint:errcheck // test cleanup
...
func TestWrite_TildePath(t *testing.T) {
	...
	defer os.RemoveAll(filepath.Join(home, ".adt-test-audit")) //nolint:errcheck // test cleanup
...
func TestWrite_AppendsBetweenCalls(t *testing.T) {
	...
	data, err := os.ReadFile(logPath) //nolint:gosec // test path derived from t.TempDir(), not user input
	if err != nil {
		t.Fatal(err)
	}
